...
function generatePromptPayPayload(target: string, amount?: number | string): string {
...
export function chatposDbPlugin(): Plugin {
  return {
    name: 'vite-plugin-chatpos-db',
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          ...
          if (url === '/api/db/auth/login' && req.method === 'POST') {
            ...
            // Allow fallback check for dev testing if matching known dev credentials
            if (!isMatch) {
              res.statusCode = 401
            ...
            if (user.role === 'pd' || role === 'pd') {
              const pdRes = await db.query(`SELECT * FROM "ProvincialDirector" WHERE "userId" = $1 LIMIT 1;`, [user.id])
              pdInfo = pdRes.rows[0] || null
            }

            if (user.role === 'agent' || role === 'agent') {
              const agRes = await db.query(`SELECT * FROM "Agent" WHERE "userId" = $1 LIMIT 1;`, [user.id])
              agentInfo = agRes.rows[0] || null
            }

            if (user.role === 'owner' || user.role === 'merchant' || role === 'merchant') {
              const stRes = await db.query(`SELECT * FROM "Store" WHERE "userId" = $1 LIMIT 1;`, [user.id])
              storeInfo = stRes.rows[0] || null
            }

            // Update lastLoginAt if AdminAccount
            if (isAdminAccount) {
              db.query(`UPDATE "AdminAccount" SET "lastLoginAt" = NOW(), "updatedAt" = NOW() WHERE id = $1`, [user.id]).catch(() => {})
            }
          ...
          if (url === '/api/db/auth/register-pd' && req.method === 'POST') {
            try {
              ...
              const cleanEmail = String(email).trim().toLowerCase()
              const existing = await db.query(`SELECT id FROM "User" WHERE LOWER(email) = $1 LIMIT 1;`, [cleanEmail])
          ...
          // ── 6. GET /api/db/stats (Dashboard Overview) ──────────────────
          ...
          if (req.method === 'POST' && url === '/api/db/transactions/create') {
            try {
              ...
              if (!targetStoreId) {
                const storeRes = await db.query('SELECT id FROM "Store" ORDER BY "createdAt" DESC LIMIT 1;')
          ...
          if (req.method === 'POST' && (url === '/api/v1/payments/qr' || url.startsWith('/api/v1/payments/qr?'))) {
            ...
            const storeRes = await db.query('SELECT id, name, phone, "qrSettings", "webhookUrl", "webhookSecret" FROM "Store" ORDER BY "createdAt" DESC LIMIT 1;')
            const store = storeRes.rows[0]
            const targetPromptPay = customPromptPay || store?.phone || (store?.qrSettings && (store.qrSettings as any).promptPayId) || '0823456789'
