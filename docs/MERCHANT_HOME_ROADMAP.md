...
| `/merchant/pos` | Payment AI product grid/cart | Product/Table API + idempotent Order create | **production order path ใช้ได้** | payment linkage/receipt และ held/split bill ยังต้องมี durable contract |
| `/merchant/orders` | Payment AI order cards/status actions | Store-scoped Order API, optimistic state transition, polling และ stock commit | **production path ใช้ได้** | SSE/event projection, payment status linkage และ mutation E2E เพิ่มเติม |
...
| `/merchant/tables` | Payment AI table cards/form/QR | Table CRUD, token, optimistic version และ order aggregates | **production path ใช้ได้** | table layout/floor plan และ hardware print workflow |
