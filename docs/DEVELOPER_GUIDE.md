...
การรองรับ remote KYC case callback ใช้ migration [`011_kyc_backoffice_callback_state.sql`](../database/migrations/011_kyc_backoffice_callback_state.sql) ซึ่งเพิ่ม `backofficeCaseId`, `lastBackofficeEventOccurredAt` และ `lastBackofficeEventId`; ต้อง apply ด้วย `npm run db:migrate` ก่อน deploy code ชุดนี้. หลัง deploy ให้ scheduler เรียก Backoffice `POST /api/internal/jobs/assignment-maintenance` ตามรอบที่กำหนด เพื่อ retry callback ที่ปลายทางล้มเหลว
