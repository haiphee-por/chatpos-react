...
- ส่ง local transaction command ผ่าน `/api/v1/transactions` (same-origin เท่านั้น; `src/chatposApi.ts` ตั้ง `DEFAULT_API_BASE_URL=''` เพื่อใช้ Next.js rewrite ใน `next.config.ts` ที่ forward ไปยัง `CHATPOS_API_URL`, ห้าม hard-code `https://chatpos.biz` ในโค้ด browser); เมื่อเปิด routing server จึง forward ไปยัง path ของ Backoffice ตาม configuration
