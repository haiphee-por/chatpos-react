  ...
  return (
    <section className={`merchant-finance-view ai-finance-view ai-finance-${mode}`} aria-labelledby="merchant-finance-title">
      ...
      <div className="merchant-finance-metrics">
        ...
        <article><span>{mode === 'wallet' ? 'ยอดเงินพร้อมใช้' : 'ยอดสุทธิวันนี้'}</span><strong>{mode === 'wallet' ? amount(summary?.availableBalance, canViewBalance && summary?.balanceStatus === 'available') : amount(summary?.todayNetAmount, canViewBalance)}</strong><small>{home?.freshness.generatedAt ? `ข้อมูล ณ ${new Date(home.freshness.generatedAt).toLocaleString('th-TH')}` : 'ยังไม่มีเวลาอ้างอิง'}</small></article>
      </div>
